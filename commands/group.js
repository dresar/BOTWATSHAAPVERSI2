...
async function handleTagAll(sock, msg, args, groupsDB, senderNumber, adminNumbers) {
    ...
    try {
        ...
        // Try to send message with mentions, fallback if permission denied
        try {
            await sock.sendMessage(from, {
                text: message,
            ...
            console.error('Error sending tagall with mentions:', sendError);
            // Fallback: send without mentions if permission denied
            const fallbackMessage = message.replace(/@\d+/g, '[Member]');
            await sock.sendMessage(from, {
                text: `${fallbackMessage}\n\n⚠️ *Catatan:* Bot tidak dapat mention semua anggota karena keterbatasan permission.`
            });
        }
